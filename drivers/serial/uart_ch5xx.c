...
static int uart_wch_init(const struct device *dev)
{
	...
	err = clock_control_on(CH5XX_CLOCK_CONTROLLER, (clock_control_subsys_t)cfg->ccfg);
	if (err < 0) {
		LOG_ERR("Failed to turn on clock: %d", err);
		return err;
	}
...
#define UART_CH5XX_INIT(index)   \
	...
	CH5XX_CLOCK_DT_INST_DEFINE(index); 	\
    static const struct uart_wch_config uart_cfg_##index = {    \
		...
		.ccfg = CH5XX_CLOCK_DT_INST_DEV_CONFIG_GET(index),  		\
